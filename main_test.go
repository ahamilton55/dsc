...
func TestGetStatus(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "20x",
			line:           []string{`"GET /our-products/shave?action=redeem-gift-card HTTP/1.1" 200`, "GET /our-products/shave?action=redeem-gift-card HTTP/1.1", "200"},
			expectedStatus: "20x",
			expectedRoute:  "",
		},
		{
			name:           "30x",
			line:           []string{`"POST /our-products/shave?action=redeem-gift-card HTTP/1.1" 301`, "POST /our-products/shave?action=redeem-gift-card HTTP/1.1", "301"},
			expectedStatus: "30x",
			expectedRoute:  "",
		},
		{
			name:           "40x",
			line:           []string{`"GET /our-products/shave?action=redeem-gift-card HTTP/1.1" 403`, "GET /our-products/shave?action=redeem-gift-card HTTP/1.1", "403"},
			expectedStatus: "40x",
			expectedRoute:  "",
		},
		{
...
func TestStats(t *testing.T) {
	log := `10.10.180.161 - 50.112.166.232, 192.33.28.238 - - - [02/Aug/2015:15:56:14 +0000]  https https https "GET /our-products HTTP/1.1" 200 35967 "-" "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/43.0.2357.81 Safari/537.36"
